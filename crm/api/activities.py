...
def get_activities(name):
	if frappe.db.exists("CRM Deal", name):
		...
		return get_org_activities(name)
...
def get_org_activities(name):
	get_docinfo('', "CRM Organization", name)
	docinfo = frappe.response["docinfo"]
	print(docinfo,"OrganizationOrganizationOrganization")
	lead_meta = frappe.get_meta("CRM Organization")
	lead_fields = {field.fieldname: {"label": field.label, "options": field.options} for field in lead_meta.fields}
	avoid_fields = [
		"converted",
		"response_by",
	...
	activities = [{
		...
		"creation": doc[0],
		"owner": doc[1],
		"data": "created this Organization",
		"is_lead": True,
	}]
