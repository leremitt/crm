<template>
    ...
    <ViewControls ref="viewControls" v-model="campaigns" v-model:loadMore="loadMore" v-model:resizeColumn="triggerResize"
      v-model:updatedPageCount="updatedPageCount" doctype="UTM Campaign" :options="{
        allowedViews: ['list', 'group_by', 'kanban'],
      }" />
    <KanbanView v-if="route.params.viewType == 'kanban'" v-model="campaigns" :options="{
      getRoute: (row) => ({
        name: 'Lead',
    ...
    <CampaignListView ref="leadsListView" v-else-if="campaigns.data && rows.length" v-model="campaigns.data.page_length_count"
      v-model:list="campaigns" :rows="rows" :columns="campaigns.data.columns" :options="{
        showTooltip: false,
        resizeColumn: true,
    ...
    <div v-else-if="campaigns.data" class="flex h-full items-center justify-center">
      <div class="flex flex-col items-center gap-3 text-xl font-medium text-gray-500">
        ...
        <span>{{ __('No {0} Found', [__('Campaiagns')]) }}</span>
    ...
    <CampaignModal v-if="showLeadModal" v-model="showLeadModal" v-model:quickEntry="showQuickEntryModal"
      :defaults="defaults" />
    <NoteModal v-if="showNoteModal" v-model="showNoteModal" :note="note" doctype="UTM Campaign" :doc="docname" />
    <TaskModal v-if="showTaskModal" v-model="showTaskModal" :task="task" doctype="UTM Campaign" :doc="docname" />
...
import CampaignListView from '../components/ListViews/CampaignListView.vue'
import CampaignModal from '../components/Modals/CampaignModal.vue'
  ...
  const rows = computed(() => {
    ...
    if (campaigns.value.data.view_type === 'group_by') {
      if (!campaigns.value?.data.group_by_field?.name) return []
      return getGroupedByRows(
        campaigns.value?.data.data,
        campaigns.value?.data.group_by_field,
      )
    } else if (campaigns.value.data.view_type === 'kanban') {
      return getKanbanRows(campaigns.value.data.data)
    } else {
      return parseRows(campaigns.value?.data.data)
    }
  })
