<template>
    <Dialog v-model="show" :options="{ size: '3xl' }">
      <template #body>
        <div class="bg-white px-4 pb-6 pt-5 sm:px-6">
          ...
          <div>
            <Fields v-if="sections.data" :sections="sections.data" :data="campaign" />
  ...
  const sections = createResource({
    ...
    cache: ['quickEntryFields', 'UTM Campaign'],
    params: { doctype: 'UTM Campaign', type: 'Quick Entry' },
    auto: true,
    transform: (data) => {
  ...
  const leadStatuses = computed(() => {
    let statuses = statusOptions('campaign')
    if (!lead.status) {
